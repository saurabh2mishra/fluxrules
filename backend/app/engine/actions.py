...
@register_action(
    name="call_drim_descision",
    description="Call an external webhook/API endpoint",
    category="integrations"
)
def call_drim_decision(event: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    """Call drim API"""
    url = context.get("url")
    method = context.get("method", "POST")
    logger.info(f"DRIM API {method} {url}")

    return {
        "drim_api_called": True,
        "url": url,
        "method": method
    }
# ============================================================================
