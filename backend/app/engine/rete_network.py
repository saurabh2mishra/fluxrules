...
class BetaNode:
    ...
    tuple_memory: Set[Tuple[int]] = field(default_factory=set)
    ...
    def add_tuple(self, fact_hash: int):
        """Store a fact tuple for stateful retraction."""
        self.tuple_memory.add((fact_hash,))

    def remove_tuples_containing(self, fact_hash: int) -> Set[Tuple[int]]:
        """Remove all tuples that reference the given fact hash."""
        removed = {tpl for tpl in self.tuple_memory if fact_hash in tpl}
        self.tuple_memory.difference_update(removed)
        return removed
