...
def create_rule(
    ...
    # Invalidate conflict cache AFTER creating the rule
...
# ---------------------------------------------------------------------------
# CSV / XLSX file upload for bulk rule import
# ---------------------------------------------------------------------------

def _parse_csv_rules(content: bytes) -> List[dict]:
    """Parse CSV bytes into a list of rule dicts compatible with RuleCreate."""
    ...
    for i, row in enumerate(reader):
        try:
            ...
            # condition_dsl — expect JSON string in CSV cell
...
def _parse_xlsx_rules(content: bytes) -> List[dict]:
    """Parse XLSX bytes into a list of rule dicts compatible with RuleCreate."""
    try:
        import openpyxl
    except ImportError:
        raise ValueError("openpyxl is not installed. Install it to support XLSX uploads: pip install openpyxl")
    ...
    if len(rows) < 2:
        raise ValueError("XLSX file must have a header row and at least one data row")

    headers = [str(h).strip().lower() if h else "" for h in rows[0]]
    required = {"name", "action", "condition_dsl"}
    ...
    for i, row in enumerate(rows[1:], start=1):
        ...
        try:
            rule: dict = {}
            rule["name"] = str(cell.get("name") or "").strip()
            if not rule["name"]:
                raise ValueError(f"Row {i+1}: 'name' is required")
            rule["description"] = str(cell.get("description") or "").strip() or None
            rule["group"] = str(cell.get("group") or "").strip() or None
            rule["priority"] = int(cell.get("priority") or 0)
            enabled_raw = str(cell.get("enabled") or "true").strip().lower()
            rule["enabled"] = enabled_raw in ("true", "1", "yes")
            rule["action"] = str(cell.get("action") or "").strip()
            if not rule["action"]:
                raise ValueError(f"Row {i+1}: 'action' is required")

            dsl_raw = str(cell.get("condition_dsl") or "").strip()
            if not dsl_raw:
                raise ValueError(f"Row {i+1}: 'condition_dsl' is required (JSON string)")
            rule["condition_dsl"] = json.loads(dsl_raw)

            meta_raw = str(cell.get("rule_metadata") or "").strip()
            rule["rule_metadata"] = json.loads(meta_raw) if meta_raw and meta_raw.lower() != "none" else None
...
def bulk_upload_rules(
    ...
    """
    Upload a CSV or XLSX file to bulk-import rules.

    **Required columns**: ``name``, ``action``, ``condition_dsl``
    **Optional columns**: ``description``, ``group``, ``priority``, ``enabled``, ``rule_metadata``

    ``condition_dsl`` and ``rule_metadata`` must contain valid JSON strings.

    Conflicting rules are automatically parked for review (same behavior as JSON bulk import).
    """
    ...
    try:
        if filename.endswith(".csv"):
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported file type. Please upload a .csv or .xlsx file."
            )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

    # Validate through Pydantic
    validated_rules: List[RuleCreate] = []
    parse_errors: List[dict] = []
    for i, raw in enumerate(parsed_rules):
        try:
            validated_rules.append(RuleCreate(**raw))
        except Exception as e:
            parse_errors.append({"index": i, "rule_name": raw.get("name", f"Row {i+1}"), "error": str(e)})

    if parse_errors and not validated_rules:
        raise HTTPException(status_code=400, detail={
            "message": "All rows failed validation",
            "errors": parse_errors
        })

    # Use the same logic as the existing JSON bulk endpoint
    service = RuleService(db)
    created_rules = []
    errors = list(parse_errors)  # carry forward parse errors
    ...
    parked_count = sum(1 for e in errors if e.get("parked"))
    error_count = sum(1 for e in errors if not e.get("parked"))

    if errors:
        return JSONResponse(
            status_code=207,
            content={
                "message": f"Parsed {len(parsed_rules)} rows: {len(created_rules)} created, {parked_count} parked, {error_count} failed",
    ...
    return {
        "message": f"All {len(created_rules)} rules created successfully",
        "total_parsed": len(parsed_rules),
        "created": created_rules,
        "created_count": len(created_rules),
        "parked_count": 0,
